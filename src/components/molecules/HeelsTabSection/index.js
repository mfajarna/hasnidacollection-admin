...
import { getHeelsDataByTypes } from '../../../redux/action';
...
const NewSection = () => {
  ...
  const {newHeels} = useSelector(state => state.heelsReducer);

  useEffect(() => {
    dispatch(getHeelsDataByTypes('New Collection'));
  }, []);

  return (
    <View style={{paddingTop: 8, paddingHorizontal: 24}}>
      {newHeels.map(itemNew => {
        return (
          <ItemListFood
            key={itemNew.id}
            type="product"
...
const PopularSection = () => {
  const navigation = useNavigation();
  const dispatch = useDispatch();
  const {popular} = useSelector(state => state.heelsReducer);

  useEffect(() => {
    dispatch(getHeelsDataByTypes('Popular'));
  }, []);
  return (
    <View style={{paddingTop: 8, paddingHorizontal: 24}}>
...
const RecommendedSection = () => {
  const navigation = useNavigation();
  const dispatch = useDispatch();
  const {recommended} = useSelector(state => state.heelsReducer);

  useEffect(() => {
    dispatch(getHeelsDataByTypes('Recommended'));
  }, []);
  return (
    <View style={{paddingTop: 8, paddingHorizontal: 24}}>
...
export default HeelsTabSection;
