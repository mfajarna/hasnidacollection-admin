...
import { getHeelsDataByTypes, getHijabDataByTypes } from '../../../redux/action';
...
const NewSection = () => {
  ...
  const {newHijab} = useSelector(state => state.categoryReducer);

  useEffect(() => {
    dispatch(getHijabDataByTypes('New Collection'));
  }, []);

  return (
    <View style={{paddingTop: 8, paddingHorizontal: 24}}>
      {newHijab.map(itemNew => {
        return (
          <ItemListFood
            key={itemNew.id}
            type="product"
...
const PopularSection = () => {
  ...
  useEffect(() => {
    dispatch(getHijabDataByTypes('Popular'));
  }, []);
  return (
    <View style={{paddingTop: 8, paddingHorizontal: 24}}>
...
const RecommendedSection = () => {
  ...
  useEffect(() => {
    dispatch(getHijabDataByTypes('Recommended'));
  }, []);
  return (
    <View style={{paddingTop: 8, paddingHorizontal: 24}}>
...
export default HijabTabSection;
